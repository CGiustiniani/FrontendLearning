...
export class CircleComponent implements AfterViewInit {
  ...
  GRID_LINES: number = 10;
  lineInterval: number = 0;
  gridColor: string = "lightgreen";
  static CANVAS_SIZE: number = 180;
  ...
  static generateRandomPoints() {
    var X = Math.floor(Math.random() * CircleComponent.CANVAS_SIZE);
    var Y = Math.floor(Math.random() * CircleComponent.CANVAS_SIZE);
    return { x: X, y: Y };
  }
