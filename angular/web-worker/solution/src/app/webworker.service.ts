...
export class WebworkerService {
   ...
   */
  public run<T>(workerFunction: (input: any) => T): Promise<T> {
   ...
   * libraries can be passed through this parameter.
   */
  public runUrl(url: string): Promise<any> {
    const worker = new Worker(url);

    const promise = this.createPromiseForWorker(worker);
   ...
   */
  private createPromiseForWorker<T>(worker: Worker) {
